...
public class AgregarLocalActivity extends BaseActivity implements LocationListener,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (hasLocationPermission()) {
            ...
            setContentView(R.layout.appu_agregar_local_activity);
            injectView();
    ...
    void initUI(Bundle savedInstanceState) {
        ...
        if (Helper.getUserAppPreference(getContext()).getType_user().equals("FREELANCER")) {
            tvTittle.setText("Agregar dirección");
        }
        else
        {
            tvTittle.setText("Agregar local");
        }
    ...
    void ADDLOCAL() {
        ...
        WsParameterAgregarLocal wsParameterAgregarLocal= new WsParameterAgregarLocal();
        wsParameterAgregarLocal.setAddress(localAdress);
        wsParameterAgregarLocal.setFull_name(localName);
        wsParameterAgregarLocal.setId_user(Helper.getUserAppPreference(getContext()).getId());
        wsParameterAgregarLocal.setLatitude(latitude);
        wsParameterAgregarLocal.setLongitude(longitude);
        wsParameterAgregarLocal.setMain(0);

        userPresenter.addLocal(Helper.getUserAppPreference(getContext()).getToken(), wsParameterAgregarLocal);
    }
